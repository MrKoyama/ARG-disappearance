(() => {
  ...
  const notifiedKey = 'hanazono-private-notification-shown-v2';
  ...
  try {
    const savedArchive = JSON.parse(localStorage.getItem('hanazono-page-archive-v1') || '[]');
    if (Array.isArray(savedArchive)) archive = savedArchive;
  } catch (error) {
    archive = [];
  }
  read = [25, 26].filter(number => archive.some(page => Number(page.number) === number));
  ...
  if (!read.includes(25) || !read.includes(26)) {
    ...
    localStorage.removeItem('hanazono-private-notification-shown-v1');
    return;
  }
